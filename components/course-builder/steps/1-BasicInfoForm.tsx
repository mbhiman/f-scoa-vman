...
export function BasicInfoForm({ initialData, onSubmit }: BasicInfoFormProps) {
    ...
    // Extract the active thumbnail value to drive the preview UI
    const currentThumbnail = form.watch("thumbnail");
    const [previewUrl, setPreviewUrl] = useState<string | null>(null);

    // Securely handle object URL creation and cleanup to prevent memory leaks
    useEffect(() => {
    ...
    return (
        <form onSubmit={form.handleSubmit(handleSubmit)} className="flex flex-col admin-card p-5 md:p-6 lg:p-8">

            <div className="border-b border-admin-border pb-4 mb-6">
                <h2 className="text-lg font-bold text-admin-fg">Basic Information</h2>
                <p className="text-sm text-admin-muted-foreground mt-0.5">Define the core identity and presentation of your course.</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
                ...
                <div className="lg:col-span-2 flex flex-col gap-5">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 items-start">
                        ...
                        <div className="flex items-center gap-3 rounded-lg border border-admin-border bg-admin-bg/50 px-4 py-2.5 mt-0 md:mt-7">
                            <input
                                type="checkbox"
                                {...form.register("is_ncvet")}
                                className="w-4 h-4 rounded border-admin-border text-admin-primary focus:ring-admin-primary bg-admin-card cursor-pointer"
                            />
                            <div className="flex flex-col">
                                <span className="text-sm font-semibold text-admin-fg leading-none">NCVET Certified</span>
                                <span className="text-[11px] text-admin-muted-foreground mt-1 leading-none">Toggle if this aligns with NCVET standards.</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <TextareaInput
                        ...
                        rows={4} // Forces a more compact default height
                                    ...
                                    {previewUrl ? (
                                        <>
                                            ...
                                            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                                <div className="flex items-center gap-1.5 bg-admin-card/90 backdrop-blur-sm text-admin-fg text-xs font-bold px-3 py-1.5 rounded-lg shadow-sm border border-admin-border">
                                        ...
                                        <div className="flex flex-col items-center justify-center p-4 text-center">
                                            <UploadCloud className="w-8 h-8 text-admin-muted-foreground/60 mb-2" />
                                            <p className="text-xs font-semibold text-admin-fg">Click to upload</p>
                                            <p className="text-[11px] text-admin-muted-foreground mt-0.5">PNG, JPG up to 5MB</p>
                                        </div>
                                    )}
            ...
            {/* Compact Footer */}
            <div className="mt-8 pt-4 flex justify-end border-t border-admin-border">
